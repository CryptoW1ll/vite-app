...
/**
 * Initiates the Kick OAuth 2.1 User Access Token flow using backend PKCE storage
 * Stores PKCE data securely on the backend server
 * @param {string[]} scopes - Array of requested permissions
 */
export const initiateKickOAuthWithBackend = async (scopes = ['user:read']) => {
  const CLIENT_ID = import.meta.env.VITE_KICK_CLIENT_ID;
  
  // Use production redirect URI for now since that's what's configured in Kick
  ...
  try {
    ...
    const storeResponse = await fetch(`${backendURL}/api/auth/kick/store-pkce`, {
      method: 'POST',
    ...
    if (!storeResponse.ok) {
      ...
      throw new Error(errorData.message || 'Failed to store PKCE data on server');
    }

    const storeResult = await storeResponse.json();
    console.log('[OAUTH] PKCE data stored successfully:', storeResult);
    ...
    console.log('Redirecting to Kick OAuth server (backend PKCE):', authUrl);
    window.location.href = authUrl;

  } catch (error) {
    console.error('Failed to initiate backend OAuth flow:', error);
